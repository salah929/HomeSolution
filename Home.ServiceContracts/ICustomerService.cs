...
namespace Home.ServiceContracts
{
    ...
    {
        Task<CustomerDto?> GetByIdAsync(Guid customerId, bool includeOrders = false, CancellationToken ct = default);
        Task<IReadOnlyList<CustomerDto>> GetAllAsync(CancellationToken ct = default);

        Task<IReadOnlyList<CustomerDto>> SearchAsync(string? searchString = null, string? searchBy = null,
        ...
        Task<IReadOnlyList<CustomerOrderDto>> GetOrdersByCustomerAsync(Guid customerId, bool includeItems = false, CancellationToken ct = default);

        Task<Guid> CreateAsync(CreateCustomerDto dto, CancellationToken ct = default);
        Task<bool> UpdateAsync(UpdateCustomerDto dto, CancellationToken ct = default);
