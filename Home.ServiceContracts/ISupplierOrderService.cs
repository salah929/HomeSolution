...
namespace Home.ServiceContracts
{
    ...
    {
        Task<SupplierOrderDto?> GetByIdAsync(Guid orderId, bool includeItems = false, CancellationToken ct = default);

        Task<IReadOnlyList<SupplierOrderDto>> SearchAsync(string? searchString, string? searchBy,
                                                          Guid? supplierId = null, SupplierOrderStatus? status = null,
                                                          DateTime? from = null, DateTime? to = null,
                                                          string? sortBy = null, bool desc = false,
                                                          int page = 1, int pageSize = 20,
                                                          CancellationToken ct = default);

        Task<int> CountAsync(string? searchString, string? searchBy, Guid? supplierId = null, SupplierOrderStatus? status = null,
                             DateTime? from = null, DateTime? to = null, CancellationToken ct = default);

        Task<Guid> CreateAsync(CreateSupplierOrderDto dto, CancellationToken ct = default);
        Task<bool> UpdateAsync(UpdateSupplierOrderDto dto, CancellationToken ct = default);
